...
sys_update() {
    ...
    if [ "$(apt update && apt list --upgradable)" ]; then
        echo "Available updates found:"
        apt update && apt list --upgradable
        echo "To update the system, run the following command:"
    ...
    if [ "$(apt-get install -s --only-upgrade)" ]; then
        echo "Outdated packages found:"
        apt-get install -s --only-upgrade
        echo "To update the outdated packages, run the following command:"
        echo "sudo apt-get install --only-upgrade"
    else
        echo "No outdated packages found."
    fi

    echo "Checking for security updates..."
    if [ "$(apt-get install -s --only-upgrade --security)" ]; then
        echo "Security updates found:"
        apt-get install -s --only-upgrade --security
        echo "To update the security updates, run the following command:"
        echo "sudo apt-get install --only-upgrade --security"
    else
        echo "No security updates found."
    fi

    echo "Checking for kernel updates..."
    if [ "$(apt-get install -s --only-upgrade linux-image-generic)" ]; then
        echo "Kernel updates found:"
        apt-get install -s --only-upgrade linux-image-generic
        echo "To update the kernel, run the following command:"
        echo "sudo apt-get install --only-upgrade linux-image-generic"
    else
        echo "No kernel updates found."
    fi

    local report_file="$REPORT_DIR/system_updatable_audit_$(date +%F_%T).txt"
    (
        echo "System Updatable Audit Report"
        echo "==================="
        echo "Generated by CPScript's System audit - https://github.com/CPScript/system-audit"
        echo ""
        echo ""
